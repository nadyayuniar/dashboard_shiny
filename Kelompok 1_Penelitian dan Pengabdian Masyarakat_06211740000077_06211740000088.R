...
title<-tags$img(src="download.png",width='150px')
...
# importing data
penits <- read.csv("D:/Data Penelitian Dosen ITS.csv")
pengits <- read.csv("D:/Data Pengabdian Masyarakat Dosen ITS.csv")
penstat <- read.csv("D:/Data Penelitian Dosen Statistika ITS.csv")
pengstat <- read.csv("D:/Data Pengabdian Masyarakat Dosen Statistika ITS.csv")
database <- read.csv("D:/Database Dosen ITS.csv")

saveDatapenits <- function(input){
  penits1 <- data.frame(cbind(as.character(input$Sk1),as.character(input$Na1),as.character(input$Ju1)))
  penits1 <- data.frame(cbind(penits1,as.integer(input$DP1)))
  penits1 <- data.frame(cbind(penits1,I(as.character(input$SU1)),I(as.character(input$De1))))
  penits1 <- data.frame(cbind(penits1,as.character(input$Ta1)))
  penits1 <- data.frame(cbind(penits1,as.character(input$SP1),as.integer(input$DD1)))
  colnames(penits1) <-colnames(penits)
  rownames(penits1)<-(nrow(penits)+1)
  print(penits1)
  write.table(penits1,"D:/Data Penelitian Dosen ITS.csv",sep=",",append=TRUE,quote=TRUE,col.names = FALSE,row.names = FALSE)
  #penits1 <-read.csv("pi.csv")[-1]
  #penits <-rbind(penits,penits1)
...
saveDatapenstat <- function(input){
  penstat1 <- data.frame(cbind(as.character(input$Sk2),as.character(input$Na2),as.character(input$Ju2)))
  penstat1 <- data.frame(cbind(penstat1,as.integer(input$DP2)))
  penstat1 <- data.frame(cbind(penstat1,I(as.character(input$SU2)),I(as.character(input$De2))))
  penstat1 <- data.frame(cbind(penstat1,as.character(input$Ta2)))
  penstat1 <- data.frame(cbind(penstat1,as.character(input$SP2),as.integer(input$DD2)))
  colnames(penstat1) <-colnames(penstat)
  rownames(penstat1)<-(nrow(penstat)+1)
  print(penstat1)
  write.table(penstat1,"D:/Data Penelitian Dosen Statistika ITS.csv",sep=",",append=TRUE,quote=TRUE,col.names = FALSE,row.names = FALSE)
  #penstat1 <-read.csv("ps.csv")[-1]
  #penstat <-rbind(penstat,penstat1)
...
saveDatapengits <- function(input){
  pengits1 <- data.frame(cbind(as.character(input$Sk3),as.character(input$Na3),as.character(input$Ju3)))
  pengits1 <- data.frame(cbind(pengits1,I(as.character(input$SU3)),I(as.character(input$De3))))
  pengits1 <- data.frame(cbind(pengits1,as.character(input$Ta3)))
  pengits1 <- data.frame(cbind(pengits1,as.character(input$SP3),as.integer(input$DD3)))
  colnames(pengits1) <-colnames(pengits)
  rownames(pengits1)<-(nrow(pengits)+1)
  print(pengits1)
  write.table(pengits1,"D:/Data Pengabdian Masyarakat Dosen ITS.csv",sep=",",append=TRUE,quote=TRUE,col.names = FALSE,row.names = FALSE)
  #pengits1 <-read.csv("pmi.csv")[-1]
  #pengits <-rbind(pengits,pengits1)
...
saveDatapengstat <- function(input){
  pengstat1 <- data.frame(cbind(as.character(input$Sk4),as.character(input$Na4),as.character(input$Ju4)))
  pengstat1 <- data.frame(cbind(pengstat1,I(as.character(input$SU4)),I(as.character(input$De4))))
  pengstat1 <- data.frame(cbind(pengstat1,as.character(input$Ta4)))
  pengstat1 <- data.frame(cbind(pengstat1,as.character(input$SP4),as.integer(input$DD4)))
  colnames(pengstat1) <-colnames(pengstat)
  rownames(pengstat1)<-(nrow(pengstat)+1)
  print(pengstat1)
  write.table(pengstat1,"D:/Data Pengabdian Masyarakat Dosen Statistika ITS.csv",sep=",",append=TRUE,quote=TRUE,col.names = FALSE,row.names = FALSE)
  #pengstat1 <-read.csv("pmi.csv")[-1]
  #pengstat <-rbind(pengstat,pengstat1)
...
server<-function(input,output,session){
  ...
  output$user1 <- renderUser({
    dashboardUser(
      name = "Nadya Yuniar Desi Prameswari", 
      src = "foto nadya.jpg", 
      footer = p("06211740000077", class = "text-center")
    )})
  
  
  output$user2 <- renderUser({
    dashboardUser(
      name = "Rizky adi Wijaya", 
      src = "foto aw.jpg", 
      subtitle = "06211740000088"
    )
  })
